...
def load_model(model_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": f"Model {model.name} loaded successfully"}
        )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def get_model_metrics(model_id):
    ...
    try:
        ...
        metrics = (
            model.metrics.order_by(model.metrics.created_at.desc()).limit(100).all()
        )

        return jsonify(
            {"status": "success", "metrics": [metric.to_dict() for metric in metrics]}
        )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
