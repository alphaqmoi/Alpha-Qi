...
class SystemMonitor:
    ...
    def init_app(self, app):
        ...
        # Load configuration
        self.metrics_interval = app.config.get('SYSTEM_MONITOR_INTERVAL', self.metrics_interval)
        self.max_history_size = app.config.get('SYSTEM_METRICS_HISTORY_SIZE', self.max_history_size)
        
        # Load resource limits
        self.resource_limits.update(app.config.get('SYSTEM_RESOURCE_LIMITS', {}))
        self.optimization_thresholds.update(app.config.get('SYSTEM_OPTIMIZATION_THRESHOLDS', {}))
    ...
    def _save_metrics(self):
        ...
        try:
            metrics_file = Path(self.app.config.get('METRICS_DIR', 'metrics')) / 'system_metrics.json'
    ...
    def _check_resource_usage(self, metrics: Dict):
        ...
        try:
            ...
            if metrics['cpu']['percent'] > self.resource_limits['cpu_percent']:
                self._add_alert('high_cpu', f"CPU usage at {metrics['cpu']['percent']}%")
            
            # Check memory usage
            if metrics['memory']['percent'] > self.resource_limits['memory_percent']:
                self._add_alert('high_memory', f"Memory usage at {metrics['memory']['percent']}%")
            
            # Check disk usage
            if metrics['disk']['percent'] > self.resource_limits['disk_percent']:
                self._add_alert('high_disk', f"Disk usage at {metrics['disk']['percent']}%")
            
            # Check swap usage
            if metrics['swap']['percent'] > self.resource_limits['swap_percent']:
                self._add_alert('high_swap', f"Swap usage at {metrics['swap']['percent']}%")
        except Exception as e:
    ...
    def _optimize_resources(self, metrics: Dict):
        ...
        try:
            ...
            if metrics['memory']['percent'] > self.optimization_thresholds['memory_percent']:
    ...
    def _optimize_cpu(self):
        ...
        try:
            ...
            if hasattr(self.app, 'thread_pool'):
                self.app.thread_pool._max_workers = max(1, self.app.thread_pool._max_workers - 1)
