...
from models import VoiceAudio, VoiceModel, VoiceSession, db
...
class VoiceManager:
    ...
    def load_model(
        self, model_id: str, config: Optional[VoiceConfig] = None
    ) -> Tuple[Any, Any]:
        ...
        try:
            ...
            if model.type == "stt":
                ...
                model_obj = WhisperForConditionalGeneration.from_pretrained(
                    model.model_id, token=model.parameters.get("api_key")
                ).to(config.device)
            else:
                # Load TTS model
    ...
    def _load_model_in_cloud(
        self, model: VoiceModel, config: VoiceConfig
    ) -> Tuple[Any, Any]:
        ...
        try:
            ...
            while True:
                ...
                if status["status"] == "completed":
                    ...
                    raise Exception(
                        f"Failed to load voice model in cloud: {status.get('error')}"
                    )
                import time
    ...
    def transcribe(
        self, audio_path: str, model_id: str, config: Optional[VoiceConfig] = None
    ) -> str:
        ...
        try:
            ...
            input_features = processor(
                waveform, sampling_rate=config.sampling_rate, return_tensors="pt"
    ...
    def synthesize(
        self, text: str, model_id: str, config: Optional[VoiceConfig] = None
    ) -> str:
        ...
        try:
            ...
            # Generate speech
            speech = model.generate_speech(inputs, max_length=config.max_length)

            # Save audio file
            session_id = str(uuid.uuid4())
            output_path = self._get_model_path(session_id, "output")

            torchaudio.save(output_path, speech.cpu(), config.sampling_rate)
    ...
    def save_audio(
        self, session_id: str, audio_path: str, audio_type: str
    ) -> VoiceAudio:
        ...
        try:
            ...
            if audio_type == "input":
                stt_model = VoiceModel.query.filter_by(
                    type="stt", is_active=True
                ).first()
                if stt_model:
