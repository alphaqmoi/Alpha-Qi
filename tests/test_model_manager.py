...
from utils.model_manager import ModelManager, ModelLoadError, ModelInferenceError
...
class TestModelManager:
    ...
    def test_load_model_success(self, model_manager, sample_model_version):
        ...
        with patch('torch.load') as mock_load, \
             patch('torch.nn.Module.load_state_dict') as mock_load_state:
            # Mock model loading
    ...
    @pytest.mark.parametrize("batch_size,expected_time", [
        (1, 0.1),
        (4, 0.3),
        (8, 0.5)
    ])
    def test_batch_inference(self, model_manager, sample_model_version, 
                           batch_size, expected_time):
        """Test batch inference with different batch sizes."""
        with patch('time.time') as mock_time, \
             patch('torch.nn.Module.eval') as mock_eval:
            # Mock timing
    ...
    def test_model_resource_limits(self, model_manager, sample_model_version):
        ...
        with patch('torch.cuda.get_device_properties') as mock_get_props, \
             patch('torch.cuda.memory_allocated') as mock_mem_alloc:
            # Mock GPU memory
    ...
    def test_model_validation(self, model_manager, sample_model_version):
        ...
        with patch('torch.nn.Module.eval') as mock_eval:
            ...
            validation_results = model_manager.validate_model(
                sample_model_version.id,
                test_data=[{'input': 'test', 'expected': 1}]
            )
