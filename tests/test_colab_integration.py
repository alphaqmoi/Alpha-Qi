...
class TestColabManager:
    ...
    @pytest.mark.parametrize("is_colab,expected", [(True, True), (False, False)])
    ...
    @pytest.mark.parametrize(
        ...
        [(True, False, "gpu"), (False, True, "mps"), (False, False, "cpu")],
    )
    ...
    def test_authenticate_success(self, mock_colab_manager):
        ...
        with patch("google.oauth2.credentials.Credentials") as mock_creds, patch(
    ...
    def test_colab_fallback(self, mock_colab_manager):
        ...
        with patch.object(manager, "check_local_resources") as mock_check, patch.object(
            manager, "connect_to_colab"
        ) as mock_connect, patch.object(manager, "sync_to_colab") as mock_sync:

            # Mock resource check to trigger fallback
            mock_check.return_value = {"should_offload": True}
    ...
    def test_sync_to_colab(self, mock_colab_manager, tmp_path):
        ...
        manager.colab_runtime = {"models_dir": str(tmp_path / "colab_models")}

        with patch("pathlib.Path.glob") as mock_glob, patch(
            "pathlib.Path.exists"
        ) as mock_exists, patch("pathlib.Path.stat") as mock_stat, patch(
    ...
    def test_check_resource_availability(self, mock_colab_manager, mock_torch):
        ...
        manager._memory_info = {"available": 8 * 1024 * 1024 * 1024}  # 8GB
        ...
        assert (
            manager.check_resource_availability(4 * 1024 * 1024 * 1024) is True
        )  # 4GB
        assert (
            manager.check_resource_availability(16 * 1024 * 1024 * 1024) is False
        )  # 16GB
    ...
    def test_cleanup_runtime(self, mock_colab_manager, mock_torch):
        ...
        with patch("torch.cuda.empty_cache") as mock_empty_cache, patch(
            "gc.collect"
        ) as mock_gc:
    ...
    @pytest.mark.parametrize(
        "resource_threshold,should_offload", [(0.5, True), (0.9, False)]
    )
    def test_resource_threshold_offloading(
        self, resource_threshold, should_offload, mock_psutil, mock_gputil
    ):
