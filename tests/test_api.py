...
class TestAuthAPI:
    ...
    def test_login_success(self, client, test_db, sample_user):
        ...
        data = {"username": sample_user.username, "password": "testpass123"}
...
class TestModelAPI:
    ...
    def test_get_model(self, client, test_db, auth_headers, sample_model):
        ...
        response = client.get(f"/api/models/{sample_model.id}", headers=auth_headers)
    ...
    def test_update_model(self, client, test_db, auth_headers, sample_model):
        """Test model update."""
        data = {"description": "Updated description", "parameters": {"size": "medium"}}
    ...
    def test_delete_model(self, client, test_db, auth_headers, sample_model):
        """Test model deletion."""
        response = client.delete(f"/api/models/{sample_model.id}", headers=auth_headers)
...
class TestModelVersionAPI:
    ...
    def test_create_version(self, client, test_db, auth_headers, sample_model):
        ...
        assert (
            ModelVersion.query.filter_by(
                model_id=sample_model.id, version="1.0.0"
            ).first()
            is not None
        )
    ...
    def test_list_versions(self, client, test_db, auth_headers, sample_model):
        ...
        response = client.get(
            f"/api/models/{sample_model.id}/versions", headers=auth_headers
        )
...
class TestInferenceAPI:
    ...
    def test_single_inference(
        self, client, test_db, auth_headers, sample_model_version, mock_model_manager
    ):
        """Test single inference request."""
        data = {"input": "test input", "parameters": {"max_length": 100}}
    ...
    def test_batch_inference(
        ...
        data = {
            "inputs": [{"input": f"test input {i}"} for i in range(3)],
            "batch_size": 2,
        }

        with patch("app.api.get_model_manager", return_value=mock_model_manager):
            mock_model_manager.batch_infer.return_value = [
                {"output": f"test output {i}"} for i in range(3)
            ]
