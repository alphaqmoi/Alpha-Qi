...
class TestFileSecurity:
    ...
    def test_file_size_limits(self, security_test_client, security_config):
        ...
        small_response = client.post(
            "/api/files", files={"file": ("small.txt", small_file)}
        )
        assert small_response.status_code == 201

        # Test large file
        large_response = client.post(
            "/api/files", files={"file": ("large.txt", large_file)}
        )
    ...
    def test_file_permissions(self, security_test_client, test_user, security_config):
        ...
        other_client, other_headers = security_test_client(
            {"id": "other-user-id", "username": "otheruser"}
        )

        # Act
        # Upload file
        upload_response = client.post(
            "/api/files", files={"file": ("test.txt", b"Test content")}, headers=headers
        )
        ...
        other_response = other_client.get(
            f"/api/files/{file_id}", headers=other_headers
        )
    ...
    def test_file_encryption(self, security_test_client, security_config):
        ...
        upload_response = client.post(
            "/api/files",
            files={"file": ("sensitive.txt", sensitive_content)},
            json={"encrypt": True},
        )
    ...
    def test_file_quarantine(self, security_test_client, security_config):
        ...
        upload_response = client.post(
            "/api/files", files={"file": ("suspicious.txt", suspicious_content)}
        )
    ...
    def test_file_scanning(self, security_test_client, security_config):
        ...
        test_files = {
            ...
            "eicar.txt": br"X5O!P%@AP[4\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*",
        }
    ...
    def test_file_versioning(self, security_test_client, test_user, security_config):
        ...
        v1_response = client.post(
            "/api/files", files={"file": ("test.txt", b"Version 1")}, headers=headers
        )
        ...
        v2_response = client.post(
            ...
            files={"file": ("test.txt", b"Version 2")},
            headers=headers,
        )
    ...
    def test_file_sharing(self, security_test_client, test_user, security_config):
        ...
        share_response = client.post(
            ...
            json={"user_id": "other-user-id", "permission": "read"},
            headers=headers,
        )

        # Try to access as shared user
        access_response = other_client.get(
            f"/api/files/{file_id}", headers=other_headers
        )
        ...
        modify_response = other_client.put(
            f"/api/files/{file_id}",
            files={"file": ("test.txt", b"Modified content")},
    ...
    def test_file_cleanup(self, security_test_client, security_config):
        ...
        # Upload file
        upload_response = client.post(
            "/api/files", files={"file": ("test.txt", b"Test content")}
        )
        file_id = upload_response.json["id"]
