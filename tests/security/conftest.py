    ...
    def _headers(user: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
        ...
        if user:
            token = jwt.encode(
                {
                    'sub': user['id'],
                    'exp': datetime.utcnow() + timedelta(seconds=SECURITY_CONFIG['auth']['token_expiry'])
                },
...
def test_user():
    ...
    hashed = bcrypt.hashpw(password.encode(), bcrypt.gensalt(SECURITY_CONFIG['data']['hash_rounds']))
    return {
        'id': 1,
        'username': 'testuser',
