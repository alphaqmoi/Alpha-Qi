...
class TestContainerSecurity:
    ...
    def test_docker_security(self, security_test_client, security_config):
        ...
        try:
            ...
            container.update(
                cpu_quota=50000, mem_limit="512m", memswap_limit="1g"  # 50% of CPU
            )
    ...
    def test_kubernetes_security(self, security_test_client, security_config):
        ...
        pod_manifest = {
            "apiVersion": "v1",
            "kind": "Pod",
            "metadata": {"name": "test-secure-pod", "namespace": "default"},
            "spec": {
                "containers": [
                    {
                        "name": "nginx",
        ...
        try:
            ...
            pod_info = k8s_client.read_namespaced_pod(
                name="test-secure-pod", namespace="default"
            )
...
class TestNetworkSecurity:
    ...
    def test_firewall_security(self, security_test_client, security_config):
        ...
        assert any(
            rule["port"] == 80 and rule["action"] == "allow" for rule in firewall_rules
        )
        assert any(
            rule["port"] == 3306 and rule["action"] == "deny" for rule in firewall_rules
        )
...
class TestInfrastructureCompliance:
    ...
    def test_terraform_security(self, security_test_client, security_config):
        ...
        assert all(
            tag in resource_config["resource"]["aws_instance"]["test"]["tags"]
...
class TestMonitoringSecurity:
    ...
    def test_logging_security(self, security_test_client, security_config):
        ...
        log_config = {
            ...
            "retention": {"period": "90d", "archive": True},
            "access_control": {"require_auth": True, "audit_logging": True},
        }
    ...
    def test_alerting_security(self, security_test_client, security_config):
        ...
        alert_config = {
            "encryption": {"enabled": True, "method": "end-to-end"},
            "notifications": {
                "channels": ["email", "slack"],
                "require_ack": True,
                "timeout": "15m",
            },
            "validation": {"rate_limiting": True, "duplicate_detection": True},
            "automation": {"enabled": True, "actions": ["block_ip", "disable_account"]},
        }
