...
class TestContainerRuntimeSecurity:
    ...
    def test_container_runtime_configuration(
        self, docker_client, security_test_client, security_config
    ):
        ...
        try:
            ...
            assert container_info["HostConfig"]["SecurityOpt"] == [
                "seccomp=unconfined",
                "apparmor=unconfined",
                "no-new-privileges",
            ]
    ...
    def test_container_security_scanning(
        ...
        test_image = docker_client.images.build(
            ...
            rm=True,
            buildargs={"SCAN_LEVEL": "high", "SCAN_TIMEOUT": "300"},
        )

        try:
            # Run security scan
            scan_result = docker_client.images.scan(
                test_image.id, scan_type="vulnerability", scan_level="high"
            )
    ...
    def test_container_resource_isolation(
        self, docker_client, security_test_client, security_config
    ):
        ...
        try:
            ...
            assert any(
                ulimit["Name"] == "nofile" and ulimit["Soft"] == 1024
                for ulimit in ulimits
            )
            assert any(
                ulimit["Name"] == "nproc" and ulimit["Soft"] == 1024
                for ulimit in ulimits
            )
        finally:
            container.stop()
...
class TestContainerImageSecurity:
    ...
    def test_image_vulnerability_scanning(
        self, docker_client, security_test_client, security_config
    ):
        ...
        # Build test image
        test_image = docker_client.images.build(
            path="tests/fixtures/test-image", tag="test-image:latest", rm=True
        )
        ...
        try:
            ...
            )  # No vulnerabilities in test image
        finally:
            docker_client.images.remove(test_image.id)

    def test_image_signing_verification(
        self, docker_client, security_test_client, security_config
    ):
        ...
        response = client.post(
            "/api/container/image/sign",
            json={"image": "test-image:latest", "signing_key": "test-key"},
        )
        ...
        verify_response = client.post(
            "/api/container/image/verify", json={"image": "test-image:latest"}
        )

        assert verify_response.status_code == 200
        assert verify_response.json["verified"] is True

    def test_image_base_security(
        self, docker_client, security_test_client, security_config
    ):
        ...
        test_image = docker_client.images.build(
            ...
            rm=True,
            buildargs={"BASE_IMAGE": "alpine:3.14", "SECURITY_SCAN": "true"},
        )
        ...
        try:
            ...
            sign_result = docker_client.images.sign(
                test_image.id, key_id=container_config["image"]["signing"]["key_id"]
            )
            assert sign_result["signed"] is True
        finally:
            docker_client.images.remove(test_image.id)

    def test_image_dependency_security(
        self, docker_client, security_test_client, security_config
    ):
        ...
        test_image = docker_client.images.build(
            ...
            rm=True,
            buildargs={"SCAN_DEPENDENCIES": "true", "UPDATE_DEPENDENCIES": "true"},
        )

        try:
            # Scan dependencies
            scan_result = docker_client.images.scan(
                test_image.id, scan_type="dependency", scan_level="high"
            )
            ...
            assert all(
                dep["vulnerabilities"] == [] for dep in scan_result["dependencies"]
            )
...
class TestKubernetesSecurity:
    ...
    def test_pod_security_context(
        self, k8s_client, security_test_client, security_config
    ):
        ...
        try:
            ...
            k8s_client.delete_namespaced_pod(name="test-pod", namespace="default")
    ...
    def test_network_policy(self, k8s_client, security_test_client, security_config):
        ...
        policy_manifest = {
            ...
            "spec": {
                ...
                "egress": [
                    {"to": [{"podSelector": {"matchLabels": {"role": "backend"}}}]}
                ],
            },
        }
        ...
        try:
            ...
            policy = k8s_client.create_namespaced_network_policy(
                namespace="default", body=policy_manifest
            )
...
class TestContainerSecretsSecurity:
    ...
    def test_secrets_management(
        ...
        # Test Docker secrets
        secret = docker_client.secrets.create(name="test-secret", data="secret-data")
        ...
        try:
            ...
            rotate_response = client.post(
                "/api/container/secrets/rotate", json={"secret_id": secret.id}
            )
    ...
    def test_kubernetes_secrets(
        ...
        try:
            ...
            secret = k8s_client.create_namespaced_secret(
                namespace="default", body=secret_manifest
            )
            ...
            # Test secret rotation
            rotate_response = client.post(
                "/api/kubernetes/secrets/rotate",
                json={"secret_name": "test-secret", "namespace": "default"},
            )

            assert rotate_response.status_code == 200
            assert rotate_response.json["rotated"] is True
        finally:
            k8s_client.delete_namespaced_secret(name="test-secret", namespace="default")
