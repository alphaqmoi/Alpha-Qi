...
from ollama_api import get_simulated_response, ollama_client
...
class EnhancedAI:
    ...
    AVAILABLE_MODELS = {
        "text-generation": [
            ...
            },
            {
                ...
                "name": "BLOOM 1.7B",
                "parameters": "1.7B",
                "description": "BigScience Large Open-science Open-access Multilingual Language Model",
            },
        ],
        "code-generation": [
            {
                "id": "codegen-350M-mono",
                "name": "CodeGen 350M",
                "parameters": "350M",
                "description": "Salesforce CodeGen for programming tasks",
            },
            {
                "id": "codegen-2B-mono",
                "name": "CodeGen 2B",
                "parameters": "2B",
                "description": "Larger Salesforce CodeGen model",
            },
            {
                "id": "starcoder",
                "name": "StarCoder",
                "parameters": "15B",
                "description": "A large language model for code",
            },
        ],
        "voice-synthesis": [
            {
                ...
                "description": "High-quality English voice synthesis",
            },
            {
                "id": "mms-tts-eng",
                "name": "MMS TTS English",
                "description": "Meta's Massively Multilingual Speech model for English",
            },
            {
                "id": "speecht5-tts",
                "name": "SpeechT5",
                "description": "Microsoft's Speech-Text model",
            },
        ],
    }
    ...
    def set_active_model(self, model_type, model_id):
        ...
        model_exists = any(
            model["id"] == model_id for model in self.AVAILABLE_MODELS[model_type]
        )
        if not model_exists:
            return False
    ...
    def summarize_text(self, text, max_sentences=5):
        ...
        top_sentences_idx = sorted(sentence_scores, key=lambda x: x[1], reverse=True)[
            :max_sentences
        ]
    ...
    def generate_voice_data(self, text, voice_id=None):
        ...
        # Find voice details
        voice = next(
            (v for v in self.AVAILABLE_VOICES if v["id"] == voice_id),
        ...
        return {"text": text, "voice": voice, "settings": settings}
    ...
    def generate_response(self, message, conversation_history=None):
        ...
        if message_lower.startswith("calculate:"):
            ...
            if calculation["success"]:
                response_text = (
                    f"The result of {expression} is: {calculation['result']}"
                )
            ...
            if "list voices" in message_lower:
                ...
                if match:
                    ...
                    if voice:
                        ...
                        reasoning = (
                            f"Changed active voice to {voice['name']} ({voice['id']})"
                        )
                    else:
            ...
            if "list models" in message_lower:
                ...
                for model_type, model_list in models.items():
                    response_text += (
                        f"## {model_type.replace('-', ' ').title()} Models\n"
                    )
                ...
                if match:
                    ...
                    if self.set_active_model(model_type, model_id):
                        ...
                        reasoning = (
                            f"Changed active model for {model_type} to {model_id}"
                        )
                    else:
        ...
        # Text generation (default)
            ...
            if any(
                ...
                for keyword in ["explain", "what is", "how does", "definition"]
            ):
                ...
                or "difference" in message_lower
