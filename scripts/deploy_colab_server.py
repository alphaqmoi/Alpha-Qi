...
class ColabServerDeployer:
    ...
    def create_notebook(self, template_name: str, output_name: Optional[str] = None) -> Path:
        ...
        # Save notebook
        output_name = output_name or f"{template_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ipynb"
...
def main():
    parser = argparse.ArgumentParser(description="Deploy and manage Colab server for AlphaQ")
    parser.add_argument("--config", default="config/colab_server.yaml",
                      help="Path to configuration file")
    parser.add_argument("--action", choices=["deploy", "status", "create-template", "list-templates"],
    ...
    try:
        if args.action == "deploy":
            ...
            if not args.server_url:
                ...
                try:
                    with open("colab_server_url.txt", "r") as f:
                        args.server_url = f.read().strip()
                except FileNotFoundError:
                    print("Error: Server URL not provided and colab_server_url.txt not found")
                    sys.exit(1)
