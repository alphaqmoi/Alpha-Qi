...
class Model(db.Model):
    ...
    type = db.Column(db.String(50), nullable=False)  # e.g., 'llm', 'embedding', 'classification'
...
class ResourceUsage(db.Model):
    ...
    request_type = db.Column(db.String(50))  # Type of request (e.g., 'inference', 'training')
...
class VoiceModel(db.Model):
    ...
    # Relationships
    stt_sessions = relationship('VoiceSession', foreign_keys='VoiceSession.stt_model_id', backref='stt_model')
    tts_sessions = relationship('VoiceSession', foreign_keys='VoiceSession.tts_model_id', backref='tts_model')
...
class VoiceSession(db.Model):
    ...
    audio_files = relationship('VoiceAudio', backref='session', cascade='all, delete-orphan')
...
class AISession(db.Model):
    ...
    status = db.Column(db.String(20), nullable=False, default='active')  # active, completed, error
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
