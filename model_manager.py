...
class ModelManager:
    ...
    def load_model(
        self, model_id: str, config: Optional[ModelConfig] = None
    ) -> Tuple[Any, Any]:
        ...
        try:
            ...
            if model.type == "code":
                model_obj = AutoModelForCausalLM.from_pretrained(
                    ...
                    quantization_config=(
                        model.quantization_config if config.quantized else None
                    ),
                )
    ...
    def _load_model_in_cloud(
        self, model: AIModel, config: ModelConfig
    ) -> Tuple[Any, Any]:
        ...
        try:
            ...
            while True:
                ...
                if status["status"] == "completed":
                    ...
                    raise Exception(
                        f"Failed to load AI model in cloud: {status.get('error')}"
                    )
                import time
    ...
    def generate_response(
        self, model_id: str, prompt: str, config: Optional[Dict[str, Any]] = None
    ) -> str:
