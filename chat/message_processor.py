...
class MessageProcessor:
    ...
    def _process_system_message(self, message_data: Dict):
        ...
        try:
            ...
            if action == "clear_history":
                ...
                self._send_response(
                    message_data.get("session_id"), {"history": history}
                )
            else:
    ...
    def _store_message_history(self, message: Dict, response: Dict):
        ...
        try:
            ...
            logger.debug(
                f"Storing history for session {session_id}: {json.dumps(history)}"
            )
        except Exception as e:
    ...
    def _send_response(self, session_id: str, response: Dict):
        ...
        try:
            ...
            # For now, just log it
            logger.debug(
                f"Sending response to session {session_id}: {json.dumps(response)}"
            )
        except Exception as e:
